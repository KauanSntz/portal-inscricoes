...
(() => {
  ...
  const HOME_PRIORITY_UNIT_KEYS = new Set(["sede", "leste", "sul", "norte", "compensa"]);
  ...
  const unitFromTitleChunk = (title) => {
    ...
    const compact = norm(clean)
      .replace(/\b(vestibular|matricula)\b/g, "")
      .replace(/\bonline\b/g, "")
      .replace(/(semipresencial|semi[-\s]?flex|flex|hibrido|presencial|ead)(2026\/1)/g, "$1 $2")
      .replace(/2026\/1/g, "")
      .trim();

    const match = compact.match(/^(.*?)\s*(100%\s*ead|ead|semipresencial|semi[-\s]?flex|flex|hibrido|presencial)\s*$/i);
  ...
  const mergePreferredUnits = (jsonRecords, portalRecords) => {
    const byPortalHome = portalRecords.filter((rec) => HOME_PRIORITY_UNIT_KEYS.has(rec.unitKey));
    const keepFromJson = jsonRecords.filter((rec) => !HOME_PRIORITY_UNIT_KEYS.has(rec.unitKey));

    const merged = [...keepFromJson, ...byPortalHome];
    const seen = new Set();
    const unique = [];

    for (const rec of merged) {
      const key = [rec.unitKey, rec.code, rec.typeKey, rec.modalityKey, rec.url || rec.rawUrl].join("|");
      if (seen.has(key)) continue;
      seen.add(key);
      unique.push(rec);
    }
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      if (fromJson.length) {
        const merged = mergePreferredUnits(fromJson, fallback);
        state.sourceLabel = `JSON bruto + unidades oficiais do Portal (${merged.length} registros)`;
        return merged;
      }
    } catch {
      // fallback
